...
def group_omission_data(args, models=['bart_base', 't5_small', 'baseline']):
    # 원래 models=['bart_large', 'bart_base', 't5_base', 't5_small', 'baseline', 'pegasus']
    raw_datasets, text_column, summary_column = get_raw_datasets(args)
    ...
    for model in models:
        for strategy_token in ['beam', 'sample']: 
            ...
            data = load_from_disk(p) #### pred 있는 게 들어가 하는뎅
            if model != 'pegasus':
